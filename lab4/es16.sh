...
# ── Bridges for VXLAN ─────────────────────────────────────
# R1
sudo ip netns exec R1 brctl addbr br-vx
sudo ip netns exec R1 ip link set br-vx up

# R2
sudo ip netns exec R2 brctl addbr br-vx
sudo ip netns exec R2 ip link set br-vx up

sudo ip netns exec R1 ip link add vxlan0 type vxlan \
    id 42 \
    local 192.168.0.1 \
    remote 192.168.0.5 \
    dstport 4789 \
    dev veth-R1-R3
sudo ip netns exec R1 ip link set vxlan0 up
sudo ip netns exec R1 brctl addif br-vx veth-R1-sw
sudo ip netns exec R1 brctl addif br-vx vxlan0

sudo ip netns exec R2 ip link add vxlan0 type vxlan \
    id 42 \
    local 192.168.0.5 \
    remote 192.168.0.1 \
    dstport 4789 \
    dev veth-R2-R3
sudo ip netns exec R2 ip link set vxlan0 up
sudo ip netns exec R2 brctl addif br-vx veth-R2-sw
sudo ip netns exec R2 brctl addif br-vx vxlan0
