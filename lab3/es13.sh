...
sudo ovs-vsctl add-port SW1 eth-H3 tag=20
...
sudo ovs-vsctl add-port SW1 eth-H4 tag=20
sudo ip link set eth-H4 up

# -----------------------
# ROUTER R1
# -----------------------
# Single physical link R1 <-> SW1 (trunk)
sudo ip link add r1-eth0 address 00:00:00:aa:aa:aa type veth peer name sw1-r1
sudo ip link set r1-eth0 netns R1
sudo ip netns exec R1 ip link set r1-eth0 up

# Trunk carries VLAN 10 and VLAN 20 toward router
sudo ovs-vsctl add-port SW1 sw1-r1 trunks=10,20
sudo ip link set sw1-r1 up

# Router subinterfaces for VLAN 10 and VLAN 20
...
sudo ip netns exec H1 ping -c3 10.0.2.2